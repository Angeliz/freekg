...
var namelist = [];
<!--获取图谱生成数据-->
$.getJSON('assets/json/poet.json', function (json) {//这里获取诗人列表
        ...
        desinfo.push(
            {
                ...
                paibie: json.poet[a].groupidentity,
                id:a
...
function createexpbutton(info,desinfo) {
    if (desinfo!=""){
        info=info+":  ";
        var b1=document.createElement("a");
        b1.innerHTML=desinfo;
        b1.setAttribute("style","margin:3px;max-width:80%;white-space:normal;");
        b1.setAttribute("class","btn btn-exp");
        var b2=document.createElement("span");
        b2.setAttribute("style","margin-top:10px;");
        var b3=document.createTextNode(info);
        b2.appendChild(b3);
